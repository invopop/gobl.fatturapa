...
func goblBillInvoiceAddLineDetails(inv *bill.Invoice, lineDetails []*LineDetail, retainedTaxes []*RetainedTax, taxSummaries []*TaxSummary) error {
	...
	for _, detail := range lineDetails {
		...
		unitPrice, err := num.AmountFromString(strings.TrimSpace(detail.UnitPrice))
		if err != nil {
			return fmt.Errorf("parsing unit price: %w", err)
		}
		totalPrice, err := num.AmountFromString(strings.TrimSpace(detail.TotalPrice))
		if err != nil {
		...
		if detail.Quantity != "" {
			quantity, err = num.AmountFromString(strings.TrimSpace(detail.Quantity))
			if err != nil {
