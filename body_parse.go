...
func goblBillInvoiceAddGeneralDocumentData(inv *bill.Invoice, doc *GeneralDocumentData) error {
	...
	// Add totals payable
	if doc.TotalAmount != "" {
		payable, err := num.AmountFromString(strings.TrimSpace(doc.TotalAmount))
		if err != nil {
	...
	// Add totals rounding
	if doc.Rounding != "" {
		rounding, err := num.AmountFromString(strings.TrimSpace(doc.Rounding))
		if err != nil {
...
func goblBillInvoiceAddStampDuty(inv *bill.Invoice, stampDuty *StampDuty) {
	...
	amount, err := num.AmountFromString(strings.TrimSpace(stampDuty.Amount))
	if err == nil {
		if inv.Charges == nil {
...
func goblBillInvoiceAddPriceAdjustments(inv *bill.Invoice, adjustments []*PriceAdjustment) {
	...
	for _, adj := range adjustments {
		amount, err1 := num.AmountFromString(strings.TrimSpace(adj.Amount))
...
func adjustTotals(inv *bill.Invoice, doc *GeneralDocumentData) error {
	...
	if doc.TotalAmount != "" {
		ft, err := num.AmountFromString(strings.TrimSpace(doc.TotalAmount))
		if err != nil {
			return err
		}
